...
ThyroidXAgent was designed to support the full thyroid ultrasound diagnostic workflow---from image input to structured report---within a single auditable framework (Fig.~\ref{fig:introduction_overview}). This section describes the data foundation, the agentic workflow and case-level evidence store, the image-analysis modules, the clinical semantic evaluation metric (ThyClinScore), the report-generation pipeline, and the reader-study design.

\subsection{Datasets and task definitions}

OpenThyroidDB was constructed to evaluate ThyroidXAgent under heterogeneous acquisition conditions, scanner types and annotation protocols. To this end, we assembled seven public and institutional datasets into a full-spectrum benchmark (Supplementary Table~\ref{tab:dataset_summary}). This composition covers diverse file formats, imaging devices and clinical tasks, allowing the system to be evaluated not only as a predictive model but as an integrated diagnostic workflow (Fig.~\ref{fig:introduction_overview}a and Supplementary Table~\ref{tab:dataset_comparison}).

Four datasets served as internal training and validation cohorts: TN3K~\cite{gong2021multi}, TN5K~\cite{zhang2025tn5000}, ThyroidXL~\cite{duong2025thyroidxl} and PKTN~\cite{sun2025clip}. Three additional datasets were reserved as independent external test sets, each serving a distinct evaluation purpose. DDTI~\cite{pedraza2015open} was used for classification only; RJH-7K, from the TN-SCUI 2020 challenge~\cite{zhou2020thyroid}, contributed 7,288 images for segmentation-only evaluation; and ZJH-2K, collected at Zhujiang Hospital, Southern Medical University (1,854 images), served for both segmentation and classification. This internal-to-external structure allows performance to be assessed under distribution shift, which remains a central source of degradation in thyroid ultrasound AI~\cite{liu2024decade}.

The segmentation task requires predicting a binary nodule mask from an ultrasound image. The benign--malignant classification task uses binary labels from TN3K, TN5K and ThyroidXL; PKTN does not contribute classification training labels. To construct the expert pool, training portions across datasets were merged into stacked training sets, with the largest stacked set containing 18,277 images (Supplementary Table~\ref{tab:stacked_seg_performance}). This stacked-training design exposes individual experts to progressively broader data distributions, so that the ensemble can benefit from complementary generalization profiles rather than relying on a single training configuration.

Two additional datasets were used for malignant-lesion stratification. The LNM dataset comprises 338 cervical lymph-node ultrasound images from a multicentre open-access database of patients with histologically confirmed papillary thyroid carcinoma, with binary labels indicating lateral lymph-node metastasis confirmed by fine-needle aspiration biopsy. The FTC/PTC subtype dataset combines 200 public images released by Dai \textit{et al.}~\cite{dai2025improving} with 496 institutional images, yielding 696 images in total. Public subtype labels were provided with the dataset; institutional labels were confirmed by post-surgical histopathology.
...
To reduce sensitivity to dataset bias~\cite{torralba2011unbiased,liu2024decade}, the image expert pool is designed as a heterogeneous ensemble rather than a single model (Supplementary Fig.~\ref{fig:ThyroidXAgent_for_seg_and_cls}). Each expert uses a DINOv3-based backbone~\cite{simeoni2025dinov3} with a task-specific lightweight head. To encourage complementary generalization profiles, experts were trained under varying configurations along three dimensions: stacked-training composition, input resolution (128, 224 and 448 pixels), and whether DINOv3 pretrained weights were loaded or the backbone was trained from scratch. For the segmentation branch, backbone dilation rates were additionally varied to produce experts with different receptive-field profiles. This heterogeneity exposes individual experts to progressively broader data distributions, so that the router can select the most reliable candidate for each case rather than relying on a single model's bias.

Within the expert pool, the segmentation branch uses a U-Net-style decoder with skip fusion and is optimized with a combined loss:
\[
\mathcal{L}_{\text{seg}} = \mathcal{L}_{\text{wBCE}}(m, \hat{m}) + \mathcal{L}_{\text{IoU}}(m, \hat{m}),
\]
where $m$ denotes the ground-truth mask, $\hat{m}$ the prediction, $\mathcal{L}_{\text{wBCE}}$ the class-weighted binary cross-entropy and $\mathcal{L}_{\text{IoU}}$ the intersection-over-union loss. The classification branch pools backbone features using global average and max pooling, followed by a compact attention head. To mitigate class imbalance, generalized logit adjustment (GLA)~\cite{menon2021longtail} is applied to the classification logits:
\[
\tilde{z}_c = z_c + \tau\log(\pi_c),
\]
where $z_c$ is the logit for class $c$, $\pi_c$ is the empirical class prior estimated from the training set, and $\tau$ is adaptively set based on the class imbalance ratio. Binary classification tasks use BCE on the adjusted logits, whereas multi-class tasks use cross-entropy on the adjusted logits. Each expert produces a class probability $p_i$; the per-expert confidence $c_i$ is taken as the maximum softmax output of the classification head for expert $i$. Before routing, task-specific quality metrics are computed across expert outputs---morphological plausibility (area, circularity, compactness) and inter-model agreement (pairwise IoU and HD95) for segmentation, and prediction uncertainty (entropy, margin) and class consensus for classification---to inform the selection. An LLM router then selects the best mask $m^*$ and classification result by reasoning over these quality metrics, per-expert confidence estimates and case-level metadata, rather than by simple confidence maximization or majority voting. When no single candidate is clearly dominant, the router may instead select a subset of experts for weighted ensemble fusion.

After the router selects $m^*$, the radiomics branch refines it via connected-component analysis to remove isolated noisy regions when the mask contains multiple disconnected components. Two-dimensional PyRadiomics descriptors~\cite{van2017computational}---including shape, intensity and texture features---are then extracted from the refined mask--image pair, yielding the radiomic descriptor vector $\mathbf{r}^*$. These descriptors are passed to AutoGluon-tabular classifiers~\cite{erickson2020autogluon}, which ensemble multiple tabular models under automated hyperparameter optimization. The resulting tabular class prediction is stored alongside the router's selection in the case-level evidence store, providing an independent interpretive signal for clinician review. To explain the predictions, SHAP analysis~\cite{lundberg2017unified} estimates global and local feature contributions from the tabular classifier, while Grad-CAM~\cite{selvaraju2017gradcam} visualizes the image regions driving the selected segmentation model's mask prediction. These two complementary attribution modalities---feature-level and spatial---are stored as structured vectors in the evidence store. Importantly, the same segmentation, radiomics, tabular classification and explanation workflow is reused across benign--malignant classification, LNM prediction and FTC/PTC subtype classification; only task-specific classifier fine-tuning and the task description supplied to the router are changed. This reuse allows the agentic workflow to be redirected to new clinical questions without rebuilding the diagnostic pipeline.
