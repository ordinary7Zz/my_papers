...
For image segmentation and benign--malignant classification, OpenThyroidDB integrates seven public and institutional ultrasound sources comprising 32,472 images, split into 18,277 training, 850 validation and 13,345 test images (Supplementary Table~\ref{tab:dataset_summary}). TN3K~\cite{gong2021multi}, TN5K~\cite{zhang2025tn5000}, ThyroidXL~\cite{duong2025thyroidxl} and PKTN~\cite{sun2025clip} served as internal training, validation and test cohorts. DDTI~\cite{pedraza2015open} was used as an independent external test set for classification. ZJH-2K, collected at Zhujiang Hospital, Southern Medical University, contained 1,854 images and was used as an independent external test set for both segmentation and classification. RJH-7K, from the TN-SCUI 2020 challenge~\cite{zhou2020thyroid}, contained 7,288 images and was used exclusively as an independent external test set for segmentation. The segmentation task predicted a binary nodule mask. The benign--malignant classification task used binary labels from TN3K, TN5K and ThyroidXL; PKTN and DDTI did not contribute classification training labels. To construct the expert pool, we merged training portions across datasets into stacked training sets, with the largest stacked set containing 18,277 images.
