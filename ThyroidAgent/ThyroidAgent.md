...
We propose ThyroidAgent, an agent-based framework for thyroid nodule ultrasound analysis that coordinates segmentation and classification experts through evidence-aware expert selection and aggregation. Unlike conventional approaches that rely on static pipelines, ThyroidAgent reasons over structured evidence derived from ultrasound images, segmentation masks, radiomics descriptors, and expert outputs to adapt expert usage on a per-case basis. Rather than replacing task-specific predictors, the LLM operates only as a routing module over expert outputs, while connected-component post-processing is applied separately for segmentation mask refinement when needed. This design improves interpretability, robustness, and adaptability across diverse ultrasound conditions.
...
\begin{figure}[htbp]
    ...
    \caption{Overview of the ThyroidAgent framework. While traditional systems use fixed pipelines, ThyroidAgent coordinates segmentation and classification experts through evidence-aware routing and aggregation based on ultrasound images and radiomics descriptors.}
...
\begin{figure}[htbp]
    ...
    \caption{Detailed workflow of the ThyroidAgent system, showing training and inference processes, including dynamic model selection, expert integration, and evidence-driven decision-making based on ultrasound images.}
    \label{fig:WorkFlow}
\end{figure}
...
We apply connected component analysis (CCA) to refine segmentation masks before downstream evidence construction when needed~\cite{liu2025shapekit}. CCA suppresses isolated noisy regions and preserves anatomically plausible connected structures, improving mask consistency for subsequent radiomics extraction and expert evidence aggregation. This refinement step is deterministic and independent of the LLM-based expert-routing module.
...
\begin{algorithmic}[1]
\Require Image $x$; expert pools $\mathcal{E}_{seg},\mathcal{E}_{cls}$
...
\State $\{M_k,s_k\}_{k=1}^{K} \gets \mathrm{RunSeg}(\mathcal{E}_{seg},x')$ 
\State $E_{seg}\gets \mathrm{SegEvidence}(\{M_k,s_k\})$ 
\State $\hat{k}\gets \mathrm{PolicySelect}(E_{seg})$; $\hat{M}\gets \mathrm{CCARefine}(M_{\hat{k}})$; $\mathcal{L}\gets\mathcal{L}\cup\{\hat{k},E_{seg}\}$
\State $r\gets \mathrm{PyRadiomics2D}(x',\hat{M})$ 
\State $\{p_m,\gamma_m\}_{m=1}^{M} \gets \mathrm{RunCls}(\mathcal{E}_{cls},x')$ 
\State $E_{cls}\gets \mathrm{ClsEvidence}(\{p_m,\gamma_m\},r)$ 
...
In Algorithm~\ref{alg:thyroidagent_inference}, $\mathrm{PolicySelect}(\cdot)$ denotes the LLM-based routing function that consumes structured evidence rather than raw images. Segmentation evidence is defined as $E_{seg}=\{q_k,s_k,d_k\}_{k=1}^{K}$, where $q_k$ is a mask-quality proxy, $s_k$ is the expert confidence, and $d_k$ summarizes disagreement with peer experts. Classification evidence is defined as $E_{cls}=\{p_m,\gamma_m,r\}_{m=1}^{M}$, where $p_m$ is the malignancy probability, $\gamma_m$ is the classifier confidence, and $r$ is the radiomics descriptor vector. In our implementation, the routing LLM uses low-temperature decoding (e.g., temperature 0.3 with bounded output length) to improve deterministic expert selection and then aggregates the selected expert outputs through a strict JSON decision schema.
...
Multiple segmentation and classification experts are trained on stacked datasets and organized into an expert pool with a registry that records validation performance, training-set scale, and input resolution.
...
As illustrated in Fig.~\ref{fig:WorkFlow} and Algorithm~\ref{alg:thyroidagent_inference}, during inference we execute candidate experts from the two tasks and summarize their outputs into compact evidence signals. The LLM router then outputs a strict JSON decision to select experts and trigger lightweight aggregation, producing the final mask and label. This workflow replaces a single fixed forward pass with evidence-aware orchestration across experts, while CCA-based mask refinement remains a separate post-processing component for segmentation masks.
